...
(function (global, factory) {
  ...
  //
  // To determine this, we use the `SAILS_LOCALS` global;
  // specifically its _environment property.  If we don't have
  // `SAILS_LOCALS` at all, we'll just assume this is running in
  // production or staging.  This avoids logging unnecessary
  // warnings and log messages to the browser inspector in the
  // production environment (e.g. on the 404 page).
  //
  // > Read more here:
  // > https://vuejs.org/v2/guide/installation.html#Development-vs-Production-Mode
  //------------------------------------------------------------
  var IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE =
    global &&
    global.SAILS_LOCALS &&
    typeof global.SAILS_LOCALS === 'object' &&
    (global.SAILS_LOCALS._environment === 'staging' ||
      global.SAILS_LOCALS._environment === 'production');
  if (!global || !global.SAILS_LOCALS) {
    IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE = true;
  }
  //------------------------------------------------------------
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    ? define(factory)
    : (global.Vue = factory(
        IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE
      ));
})(this, function (IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE) {
  'use strict';
  ...
  var config = {
     ...
     */
    productionTip: !IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE,

    /**
     * Whether to enable devtools
     */
    devtools: !IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE,
  ...
  function initExtend(Vue) {
    ...
    Vue.extend = function (extendOptions) {
      ...
      if (!IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE && name) {
    ...
    setTimeout(function () {
      if (config.devtools) {
        if (devtools) {
          ...
          // Silently ignore missing devtools extension.
        }
      }
      if (
        !IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE &&
        ...
        console[console.info ? 'info' : 'log'](
          'You are running Sails + Vue.js in development mode.\n' +
            '(To enable production mode for Vue.js, lift Sails in production.)\n' +
            ' [?] More tips at https://sailsjs.com/docs/concepts/deployment'
        );
      }
    }, 0);
  }
  ...
  function processSlot(el) {
    if (el.tag === 'slot') {
      ...
      if (!IS_VUE_RUNNING_IN_PRODUCTION_OR_STAGING_OR_ON_ERR_PAGE && el.key) {
        warn$2(
