...
(function (factory, exposeUMD) {
  ...
  function (Vue, _, VueRouter, $, bowser) {
    ...
    function _wrapMethodsAndVerifyNoArrowFunctions(
      ...
      var lowercasedRecommendationsByKey = _.reduce(
        RECOMMENDATIONS_BY_UNRECOGNIZED_KEY,
        function (memo, correctAlias, incorrectKey) {
          ...
          if (
            ...
            throw new Error(
              'Detected unrecognized and potentially confusing key "' +
                ...
                lowercasedRecommendationsByKey[propertyName.toLowerCase()] +
                '"?'
            );
          }
        }
      ...
      if (
        ...
        _.each(
          _.difference(_.keys(def), LEGAL_TOP_LVL_KEYS),
    ...
    parasails.registerComponent = function (componentName, def) {
      ...
      if (def.methods && def.methods.goto) {
        throw new Error(
          "Component definition contains `methods` with a `goto` key-- but you're not allowed to override that"
        );
      }
      if (def.methods && def.methods.gotoAndReplaceHistory) {
        throw new Error(
          "Component definition contains `methods` with a `gotoAndReplaceHistory` key-- but you're not allowed to override that"
        );
      }
      def.methods = def.methods || {};
    ...
    parasails.registerPage = function (pageName, def) {
      ...
      if ($) {
        ...
        isWithinIgnoredElements =
          $('#' + pageName)
            .parents()
            .filter('[parasails-has-no-page-script]').length >= 1;
      } else {
      ...
      if (
        !window.document.getElementById(pageName) ||
        isWithinIgnoredElements
      ) {
        return;
      }
      ...
      if (def.data && def.data.pageName) {
        throw new Error(
          "Page script definition contains `data` with a `pageName` key, but you're not allowed to override that"
        );
      }
      def.data = _.extend(
        {
      ...
      if (def.methods && def.methods.goto) {
        throw new Error(
          "Page script definition contains `methods` with a `goto` key-- but you're not allowed to override that"
        );
      }
      if (def.methods && def.methods.gotoAndReplaceHistory) {
        throw new Error(
          "Page script definition contains `methods` with a `gotoAndReplaceHistory` key-- but you're not allowed to override that"
        );
      }
      def.methods = def.methods || {};
      if (VueRouter) {
        ...
        def.methods.goto = function () {
          throw new Error(
            "Cannot use .goto() method because, at the time when this page script was registered, VueRouter did not exist on the page yet. (If you're using Sails, please check dependency loading order in pipeline.js and make sure VueRouter is getting brought in before `parasails`.)"
          );
        };
        def.methods.gotoAndReplaceHistory = function () {
          throw new Error(
            "Cannot use .gotoAndReplaceHistory() method because, at the time when this page script was registered, VueRouter did not exist on the page yet. (If you're using Sails, please check dependency loading order in pipeline.js and make sure VueRouter is getting brought in before `parasails`.)"
          );
        };
      }
      ...
      if (def.virtualPages === undefined) {
        ...
        throw new Error(
          "Cannot use `virtualPages` because the specified value doesn't match any recognized meaning.  Please specify either `true` (for the default handling) or a dictionary of client-side routing rules."
        );
      } //ﬁ
      ...
      if (def.router && !VueRouter) {
        throw new Error(
          "Cannot use `router`, because that depends on the Vue Router.  But `VueRouter` does not exist on the page yet.  (If you're using Sails, please check dependency loading order in pipeline.js and make sure the VueRouter plugin is getting brought in before `parasails`.)"
        );
      }
      ...
      if (def.virtualPages === true) {
        if (!VueRouter) {
          throw new Error(
            "Cannot use `virtualPages`, because it depends on the Vue Router.  But `VueRouter` does not exist on the page yet.  (If you're using Sails, please check dependency loading order in pipeline.js and make sure the VueRouter plugin is getting brought in before `parasails`.)"
          );
        }
  ...
  function (global, factory) {
    ...
    //‡AMD ˚¸
      ...
      define([], function () {
        ...
        if (!global.Vue) {
          throw new Error(
            "`Vue` global does not exist on the page yet. (If you're using Sails, please check dependency loading order in pipeline.js and make sure the Vue.js library is getting brought in before `parasails`.)"
          );
        }
        Vue = global.Vue;
        if (!global._) {
          throw new Error(
            "`_` global does not exist on the page yet. (If you're using Sails, please check dependency loading order in pipeline.js and make sure the Lodash library is getting brought in before `parasails`.)"
          );
        }
        _ = global._;
        // optional deps:
