...
(function (factory, exposeUMD) {
  ...
  function (_, io, $, SAILS_LOCALS, location, File, FileList, FormData) {
    ...
    function _representsOneOrMoreFiles(that) {
      ...
      return (
        ...
        ((File ? that instanceof File : false) ||
          ...
          (_.isArray(that) &&
            that.length > 0 &&
            _.all(that, function (item) {
              return File ? _.isObject(item) && item instanceof File : false;
            })) ||
          (File
            ? _.isObject(that) &&
              _.isObject(that.file) &&
              that.file instanceof File
            : false) ||
          (_.isArray(that) &&
            that.length > 0 &&
            _.all(that, function (item) {
    ...
    function _verifyErrorNegotiationRule(negotiationRule) {
      ...
      if (
        _.isNumber(negotiationRule) &&
        Math.floor(negotiationRule) === negotiationRule
      ) {
        if (negotiationRule > 599 || negotiationRule < 0) {
          throw new Error(
            'Invalid error negotiation rule: `' +
              negotiationRule +
              '`.  If a status code is provided, it must be between zero and 599.'
          );
        }
        ...
        if (negotiationRule === undefined || _.isFunction(negotiationRule)) {
          suffix =
            "  Looking to tolerate or intercept **EVERY** error?  This usually isn't a good idea, because, just like some try/catch usage patterns, it could mean swallowing errors unexpectedly, which can make debugging a nightmare.";
        }
        throw new Error(
          'Invalid error negotiation rule: `' +
            negotiationRule +
            '`.  Please pass in a valid intercept rule string.  An intercept rule is either (A) the name of an exit or (B) a whole number representing the status code like `404` or `200`.' +
            suffix
        );
      }
    }
    ...
    Cloud.setup = function (options) {
      ...
      if (
        !_.isObject(options.methods) ||
        _.isArray(options.methods) ||
        _.isFunction(options.methods)
      ) {
        throw new Error(
          'Cannot .setup() Cloud SDK: `methods` must be provided as a dictionary of addresses and definitions.'
        );
      } //•
      ...
      if (!options.apiBaseUrl) {
        if (location) {
          ...
          throw new Error(
            'Cannot .setup() Cloud SDK: Since a location cannot be determined, `apiBaseUrl` must be provided as a string (e.g. "https://example.com").'
          );
        }
      } //ﬁ
      ...
      if (options.methods.on) {
        throw new Error(
          'Cannot .setup() Cloud SDK: `.on()` is reserved.  It cannot be used as the name for a method.'
        );
      }
      if (options.methods.off) {
        throw new Error(
          'Cannot .setup() Cloud SDK: `.off()` is reserved.  It cannot be used as the name for a method.'
        );
      }

      // Interpret methods
      var methods = _.reduce(
        options.methods,
        function (memo, appLevelSdkEndpointDef, methodName) {
          if (methodName === 'setup') {
            console.warn(
              '"setup" is a confusing name for a cloud action (it conflicts with a built-in feature of this SDK itself).  Would "initialize()" work instead?  (Continuing this time...)'
            );
          }
          ...
          if (typeof appLevelSdkEndpointDef === 'function') {
            ...
            if (
              appLevelSdkEndpointDef &&
              typeof appLevelSdkEndpointDef === 'object'
            ) {
              ...
              _verbToCheck = appLevelSdkEndpointDef.replace(
                /^\s*([^\/\s]+)\s*\/.*$/,
                '$1'
              );
              _urlToCheck = appLevelSdkEndpointDef.replace(
                /^\s*[^\/\s]+\s*\/(.*)$/,
                '/$1'
              );
            } else {
              throw new Error(
                'CloudSDK endpoint (`' +
                  methodName +
                  '`) is invalid:  Endpoints should be defined as either (1) a string like "GET /foo", (2) a dictionary containing a `verb` and a `url`, or (3) a function that returns a dictionary like that.'
              );
            }
            ...
            if (typeof _verbToCheck !== 'string' || _verbToCheck === '') {
              throw new Error(
                'CloudSDK endpoint (`' +
                  methodName +
                  "`) is invalid:  An endpoint's `verb` should be defined as a non-empty string."
              );
            }
            ...
            if (typeof _urlToCheck !== 'string' || _urlToCheck === '') {
              throw new Error(
                'CloudSDK endpoint (`' +
                  methodName +
                  "`) is invalid:  An endpoint's `url` should be defined as a non-empty string."
              );
            }
          }
          ...
          var _helpCallCloudMethod = function (argins) {
            ...
            var deferred = {
              ...
              headers: function (_customRequestHeaders) {
                if (!_.isObject(_customRequestHeaders)) {
                  throw new Error(
                    'Invalid request headers: Must be specified as a dictionary, where each key has a string value.'
                  );
                }
                requestInfo.headers = _.extend(
                  requestInfo.headers || {},
                ...
                if (typeof _protocolNameOrInstance === 'string') {
                  switch (_protocolNameOrInstance) {
                    case 'jQuery':
                      ...
                      if ($ === undefined) {
                        throw new Error(
                          'Could not access jQuery: `$` is undefined.'
                        );
                      } else {
                    ...
                    case 'io.socket':
                      ...
                      if (typeof io === 'undefined') {
                        throw new Error(
                          'Could not access `io.socket`: `io` is undefined.'
                        );
                      } else if (typeof io !== 'function') {
                        throw new Error(
                          'Could not access `io.socket`: `io` is invalid:' + io
                        );
                      } else if (typeof io.socket === 'undefined') {
                        throw new Error(
                          'Could not access `io.socket`: `io` does not have a `socket` property.  Make sure `sails.io.js` is being injected in a <script> tag!'
                        );
                      } else {
                    ...
                    default:
                      throw new Error(
                        'Unrecognized protocol: `' +
                          ...
                          '`. Use "jQuery" or "io.socket".'
                      );
                  }
                } else if (
                  _.isObject(_protocolNameOrInstance) ||
                  _.isFunction(_protocolNameOrInstance)
                ) {
                  if (_protocolNameOrInstance.name === 'jQuery') {
                    requestInfo.protocolName = 'jQuery';
                    requestInfo.protocolInstance = _protocolNameOrInstance;
                  } else if (
                    _protocolNameOrInstance.constructor.name === 'SailsSocket'
                  ) {
                    requestInfo.protocolName = 'io.socket';
                    requestInfo.protocolInstance = _protocolNameOrInstance;
                  } else if (
                    _protocolNameOrInstance.toString() ===
                      '[Package: machinepack-http]' ||
                    _protocolNameOrInstance.toString() ===
                      '[Package: sails.helpers.http]'
                  ) {
                  ...
                  // FUTURE: maybe native Node "http"?
                    ...
                    throw new Error(
                      'Unrecognized instance provided to `.protocol()`: `' +
                        _protocolNameOrInstance +
                        '`'
                    );
                  }
              ...
              intercept: function (negotiationRule, handler) {
                ...
                if (!_.isFunction(handler)) {
                  throw new Error(
                    'Invalid 2nd argument to `.intercept()`.  Expecting a handler function.'
                  );
                }
              ...
              tolerate: function (_negotiationRuleMaybe, _handlerMaybe) {
                ...
                var negotiationRule;
                if (
                  _handlerMaybe === undefined &&
                  _.isFunction(_negotiationRuleMaybe)
                ) {
                ...
                if (handler !== undefined && !_.isFunction(handler)) {
                  throw new Error(
                    'Invalid 2nd argument. to `.tolerate()`.  Expecting a handler function.'
                  );
                }
              ...
              toPromise: function () {
                if (typeof Promise === 'undefined') {
                  throw new Error(
                    'Cannot use this approach: `Promise` constructor not available in current environment.'
                  );
                }
              ...
              exec: function (exitCallbacks) {
                if (exitCallbacks) {
                  if (
                    !_.isObject(exitCallbacks) &&
                    !_.isFunction(exitCallbacks)
                  ) {
                    throw new Error(
                      'If specified, the argument passed to `.exec()` must be a dictionary containing a `success` and `error` callback.  Alternatively, you can use a Node.js-style callback.'
                    );
                  } else if (
                    _.isObject(exitCallbacks) &&
                    exitCallbacks.success &&
                    !_.isFunction(exitCallbacks.success)
                  ) {
                    throw new Error(
                      'If specified, `success` callback must be a function.'
                    );
                  } else if (
                    _.isObject(exitCallbacks) &&
                    exitCallbacks.error &&
                    !_.isFunction(exitCallbacks.error)
                  ) {
                    throw new Error(
                      'If specified, `error` callback must be a function.'
                    );
                  }
                }
                ...
                var errorInstance = new Error(
                  'Endpoint (`' +
                    methodName +
                    '`) responded with an error (or the request failed).'
                );
                ...
                if (
                  _.isObject(SAILS_LOCALS) &&
                  typeof SAILS_LOCALS._csrf !== 'undefined'
                ) {
                ...
                (function _makeAjaxCallWithAppropriateProtocol(proceed) {
                    ...
                    if (requestInfo.verb.match(/get/i)) {
                      throw new Error(
                        'Detected File or FileList instance(s) provided for parameter(s):  ' +
                          ...
                          '\n' +
                          'But this is a nullipotent (' +
                          requestInfo.verb.toUpperCase() +
                          ') ' +
                  ...
                  switch (requestInfo.protocolName) {
                    ...
                    //  ╚═▀▀▀══╝╚═╝╚═╝  ╚═╝ ╚════╝ ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═╝╚═╝
                    case 'jQuery':
                      return (function _doAjaxWithJQuery() {
                        ...
                        // > to get it to be losslessly encoded for use in backend code.
                            ...
                            function (value, fieldName) {
                              ...
                              if (_.isString(value)) {
                                ...
                                if (requestInfo.headers['X-JSON-MPU-Params']) {
                                  requestInfo.headers['X-JSON-MPU-Params'] +=
                                    ',' + fieldName;
                                } else {
                                  requestInfo.headers['X-JSON-MPU-Params'] =
                                    fieldName;
                                ...
                                try {
                                  ...
                                  var errMsgPrefix =
                                    'Could not encode value provided for ' +
                                    fieldName +
                                    ' because the value is (or contains) ';
                                  var errMsgSuffix =
                                    '.  In a request that contains one or more file uploads, any additional text parameter values need to be encoded in such a way that they can be losslessly parsed by the Sails framework.\n [?] Unsure?  Reach out at https://sailsjs.com/support';
                                  throw new Error(
                                    errMsgPrefix +
                                      'data that cannot be stringified as JSON (usually, this means it contains circular references-- i.e. its properties or array items are actually references to itself, or each other)' +
                                      errMsgSuffix
                                  );
                                }
                          ...
                          _.each(
                            uploadsByFieldName,
                            function (fileOrFileList, fieldName) {
                              ...
                              if (!_representsOneOrMoreFiles(fileOrFileList)) {
                                throw new Error(
                                  'Cannot upload as "' +
                                    fieldName +
                                    "\" because the provided value is not a File instance, an array of File instances, a dictionary like `{file: someFileInstance, name: 'filename-override.png'}`, or an array of such wrapper dictionaries.  Instead, got: " +
                                    fileOrFileList +
                              ...
                              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                              if (
                                _.isArray(fileOrFileList) ||
                                (_.isObject(fileOrFileList) &&
                                  _.isObject(fileOrFileList.constructor) &&
                                  fileOrFileList.constructor.name ===
                                    'FileList')
                              ) {
                                for (
                                  var i = 0;
                                  i < fileOrFileList.length;
                                  i++
                                ) {
                                  if (fileOrFileList[i] instanceof File) {
                                    ajaxOpts.data.append(
                                      fieldName,
                                      fileOrFileList[i],
                                      fileOrFileList[i].name
                                    );
                                  } else {
                                    ajaxOpts.data.append(
                                      fieldName,
                                      fileOrFileList[i].file,
                                      fileOrFileList[i].name ||
                                        fileOrFileList[i].file.name
                                    );
                                  }
                                } //∞
                              } else {
                        ...
                        // Otherwise, attach params as a JSON-encoded request body.
                          ...
                          // > including `NaN`, etc.
                          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                          var sanitizedTPBFN = _.mapValues(
                            textParamsByFieldName,
                            function (value) {
                              var sanitizedValue;
                              if (_.isArray(value)) {
                    ...
                    //
                    case 'io.socket':
                      return (function _doAjaxWithSocket() {
                        ...
                        if (File && requestInfo.params) {
                          _.each(
                            ...
                            function (value, fieldName) {
                              if (_representsOneOrMoreFiles(value)) {
                                throw new Error(
                                  'Detected File-like data provided for the "' +
                                    fieldName +
                                    "\" parameter -- but file uploads are not currently supported using WebSockets / Socket.io.  Please call this method using a different request protocol (e.g. `protocol: 'jQuery'`)"
                                );
                              }
                            }
                          );
                    ...
                    //  ██║ ╚═╝ ██║██║           ██║  ██║   ██║      ██║   ██║
                    //  ╚═╝     ╚═╝╚═╝           ╚═╝  ╚═╝   ╚═╝      ╚═╝   ╚═╝
                    case 'machinepack-http':
                      return (function _doAjaxWithMpHttp() {
                            ...
                            function (value, fieldName) {
                              if (_representsOneOrMoreFiles(value)) {
                                throw new Error(
                                  'Detected File-like data provided for the "' +
                                    fieldName +
                                    '" parameter -- but file uploads are not currently supported in Cloud SDK when using "machinepack-http".  Please call this method using a different request protocol.'
                                );
                              }
                            }
                          ); //∞
                        ...
                        requestInfo.protocolInstance.sendHttpRequest
                          .with(mpHttpOpts)
                          .switch({
                            error: function (err) {
                              return proceed(err);
                            ...
                            success: function (serverResponse) {
                              ...
                              try {
                                serverResponse.body = JSON.parse(
                                  serverResponse.body
                                );
                              } catch (err) {
                    ...
                    default:
                      throw new Error(
                        'Consistency violation: Unexpected protocol name received (`' +
                          requestInfo.protocolName +
                          '`)-- but it should have already been checked!'
                      );
                  } //</switch(protocol)>
                })(function afterwards(err, responseInfo) {
                  if (err) {
                    throw new Error(
                      'Consistency violation: Unexpected error in CloudSDK. Details: ' +
                        err.stack
                    );
                  }
                  ...
                  var xExitResponseHeaderValue =
                    responseInfo.headers['x-exit'] ||
                    responseInfo.headers['X-Exit'];
                  // ^This "either-or-ing" is likely necessary because of different jQuery versions.

                  if (xExitResponseHeaderValue === '_offline') {
                    console.warn(
                      'Unconventional exit detected:  `_offline` is a reserved exit name for use on the front-end, and should not be used willy nilly.  Instead, please come up with a different exit name for this scenario.'
                    );
                  } //ﬁ
                  ...
                  var matchingLifecycleInstruction = _.find(
                    requestInfo.lifecycleInstructions,
                    function (lifecycleInstruction) {
                      if (lifecycleInstruction.rule === undefined) {
                        ...
                        responseInfo.statusCode === lifecycleInstruction.rule
                      ) {
                        return true;
                      } else if (
                        responseInfo.exit === lifecycleInstruction.rule
                      ) {
                        return true;
                      }
                      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                  ...
                  (function _runInterceptOrTolerationMaybe(proceed) {
                    ...
                    if (
                      matchingLifecycleInstruction.handler.constructor.name ===
                      'AsyncFunction'
                    ) {
                      return proceed(
                        new Error(
                          '`async` functions are not *yet* fully supported in intercept/tolerate'
                        )
                      );
                      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                      ...
                      try {
                        ...
                        resultFromHandler =
                          matchingLifecycleInstruction.handler(errorInstance);
                      } catch (err) {
                        if (err === false) {
                          return proceed(undefined, true);
                        } //« special case (`throw false`)
                        else {
                          return proceed(err);
                        }
                      }
                    ...
                    if (err) {
                      throw new Error(
                        'The provided custom intercept/tolerate logic threw an unexpected, uncaught error: ' +
                          err.stack
                      );
                    ...
                    if (matchingLifecycleInstruction) {
                      if (
                        ...
                        throw new Error(
                          'Unexpected intercept/tolerate logic matched a 2xx/success response, but these methods should only be used for exceptions!'
                        );
                        // FUTURE: better error handling for this case ^^
                      }
                    }
                    ...
                    var tolerateAsIfSuccess =
                      matchingLifecycleInstruction &&
                      matchingLifecycleInstruction.type === 'tolerate';

                    // If a matching `.intercept()` was encountered, then consider whatever the intercept handler
                    // returned to be our new Error.
                    if (
                      matchingLifecycleInstruction &&
                      matchingLifecycleInstruction.type === 'intercept'
                    ) {
                      if (!_.isError(resultFromInterceptOrTolerate)) {
                        throw new Error(
                          'Unexpected value returned from .intercept() handler.  Expected an Error instance but instead, got: ' +
                            resultFromInterceptOrTolerate
                        );
                        // FUTURE: better error handling for this case ^^
                      }
                    ...
                    // If a dictionary of callbacks was provided...
                      ...
                      if (
                        responseInfo.exit !== 'error' &&
                        exitCallbacks[responseInfo.exit]
                      ) {
                        // If there's a response body, pass it to the callback.
                        if (responseInfo.body !== undefined) {
                          return exitCallbacks[responseInfo.exit](
                            tolerateAsIfSuccess
                              ? resultFromInterceptOrTolerate
                              : responseInfo.body,
                            responseInfo
                      ...
                      // Or if there isn't an error callback, just throw.
                        ...
                        errorInstance.stack =
                          UNHANDLED_ERR_PREFIX_MSG + errorInstance.stack;
                        errorInstance.responseInfo = responseInfo;
                        throw errorInstance;
                      }
                    } //‡
                    ...
                    // If _no callbacks of any kind_ were provided...
                      ...
                      throw new Error(
                        'Invalid usage of Cloud.*() method.  Provide either a dictionary of callbacks, a single callback function, or NOTHING to `.exec()`.'
                      );
                    }
            ...
            // }
            if (typeof appLevelSdkEndpointDef === 'function') {
              var returnedFromEndpointDefFn = appLevelSdkEndpointDef.apply(
                this,
                argins
              );
              if (typeof returnedFromEndpointDefFn !== 'object') {
                throw new Error(
                  'Consistency violation: Function for CloudSDK endpoint (`' +
                    methodName +
                    '`) returned an invalid result.  The return value of the specified function is not a dictionary!  If a function is supplied for an endpoint definition, it must return a dictionary containing a `verb` and a `url`.  The returned dictionary may also contain dynamic, per-request header & parameter values.'
                );
              }
              ...
              if (!_.isUndefined(returnedFromEndpointDefFn.protocol)) {
                deferred = deferred.protocol(
                  returnedFromEndpointDefFn.protocol
                );
              } else if (options.protocol) {
                deferred.protocol(options.protocol);
              } else {
                deferred.protocol(DEFAULT_PROTOCOL_NAME);
              }
            ...
            // e.g. "POST /api/v1/lawnmowers/foo/inputs/bar"
              ...
              requestInfo.verb = appLevelSdkEndpointDef.replace(
                /^\s*([^\/\s]+)\s*\/.*$/,
                '$1'
              );
              requestInfo.url = appLevelSdkEndpointDef.replace(
                /^\s*[^\/\s]+\s*\/(.*)$/,
                '/$1'
              );
              requestInfo.params = argins;
              ...
              throw new Error(
                'Consistency violation: Something happened to CloudSDK endpoint (`' +
                  methodName +
                  '`).  This was not noticed initially when building up CloudSDK endpoints, but this endpoint is now invalid.  Endpoints should be defined as either (1) a string like "GET /foo", (2) a dictionary containing a `verb` and a `url`, or (3) a function that returns a dictionary like that.'
              );
            }
            ...
            requestInfo.url.replace(
              ...
              function ($all, $1) {
                ...
                if ($1.match(/\?$/)) {
                  if (
                    requestInfo.params &&
                    requestInfo.params[routeParamName]
                  ) {
                ...
                // Mandatory:
                  ...
                  if (
                    !requestInfo.params ||
                    requestInfo.params[routeParamName] === undefined
                  ) {
                    throw new Error(
                      'Missing required param: `' + routeParamName + '`'
                    );
                  }
                  routeParameters[routeParamName] =
                    requestInfo.params[routeParamName];
            ...
            requestInfo.params = _.omit(
              requestInfo.params,
              _.keys(routeParameters)
            );
            ...
            requestInfo.url = requestInfo.url.replace(
              ...
              function ($all, $1) {
                var routeParamName = $1.replace(/^\:/, '').replace(/\??$/, '');
                if (routeParameters[routeParamName] === undefined) {
                  return '';
                }
            ...
            if (!requestInfo.verb) {
              throw new Error(
                'CloudSDK endpoint (`' +
                  methodName +
                  '`) is invalid: No HTTP verb specified.  Please specify an HTTP verb (e.g. `GET`, `POST`, etc.)'
              );
            }
          ...
          memo[methodName] = function () {
            ...
            var argins = _.reduce(
              arguments,
              function (argins, argin, i) {
                if (!appLevelSdkEndpointDef.args[i]) {
                  throw new Error(
                    'Invalid usage with serial arguments: Received unexpected ' +
                      (i === 0
                        ? 'first'
                        : i === 1
                        ? 'second'
                ...
                if (appLevelSdkEndpointDef.args[i] === '{*}') {
                  if (
                    ...
                    throw new Error(
                      'Invalid usage with serial arguments: If provided, expected ' +
                        (i === 0
                          ? 'first'
                          : i === 1
                          ? 'second'
                          ...
                          ? 'third'
                          : i + 1 + 'th') +
                        ' argument to be a dictionary (plain JavaScript object, like `{}`).  But instead, got: ' +
                        argin +
                        ''
                    );
                  } else if (
                    argin !== undefined &&
                    _.intersection(_.keys(argins), _.keys(argin)).length > 0
                  ) {
                    throw new Error(
                      'Invalid usage with serial arguments: If provided, expected ' +
                        (i === 0
                          ...
                          ? 'third'
                          : i + 1 + 'th') +
                        ' argument to have keys which DO NOT overlap with other already-configured argins!  But in reality, it contained conflicting keys: ' +
                        _.intersection(_.keys(argins), _.keys(argin)) +
                        ''
                    );
                  }
      ...
      Cloud.on = function (socketEventName, handleSocketMsg) {
        if (!socketEventName || !_.isString(socketEventName)) {
          throw new Error(
            'Invalid usage for `Cloud.on()`: Must pass in a valid first argument (a string; the name of the socket event to listen for -- i.e. the variety of incoming WebSocket messages to receive and handle).'
          );
        }
        if (!handleSocketMsg) {
          throw new Error(
            'Invalid usage for `Cloud.on()`: Must pass in a second argument (the function to run every time this WebSocket event is received).'
          );
        }

        if (!io || !io.socket) {
          throw new Error(
            'Could not bind a cloud event listener with `Cloud.on()`: WebSocket support is not currently available (`io.socket` is not available).  Make sure `sails.io.js` is being injected in a <script> tag!'
          );
        }

        var actualHandler;
        if (
          _.isObject(handleSocketMsg) &&
          !_.isArray(handleSocketMsg) &&
          !_.isFunction(handleSocketMsg)
        ) {
          ...
          actualHandler = function (msg) {
            ...
            if (_.contains(_.keys(handleSocketMsg), msg.verb)) {
              ...
              throw new Error(
                'Unhandled "' +
                  socketEventName +
                  '" cloud event:  Received an incoming WebSocket message with an unrecognized "verb" property: "' +
                  msg.verb +
                  '".  If this was deliberate, register another key in the call to `Cloud.on(\'' +
                  socketEventName +
                  "', {…, " +
                  msg.verb +
                  ': (msg)=>{…} })` to recognize this new sub-category of cloud event and handle it accordingly.  Otherwise, if you\'d like to silently ignore messages with other "verb"s (or no "verb" at all), then pass a function in to Cloud.on(), instead of a dictionary -- or register a "*" key as a catchall, and make its function a no-op.'
              );
            }
            ...
            try {
              ...
              err.message =
                'An uncaught error was thrown while handling an incoming WebSocket message (a "' +
                socketEventName +
                '" cloud event).  ' +
                err.message;
              throw err;
          ...
          throw new Error(
            'Invalid usage for `Cloud.on()`: Second argument must either be a function (the function to run every time this socket event is received) or a dictionary of functions that will be negotiated and routed to based on the incoming message\'s conventional "verb" property (e.g. `{ "bankWireReceived": (msg)=>{…}, "destroyed": (msg)=>{…}, "*": (msg)=>{…} }`.'
          );
        }
      ...
      Cloud.off = function (socketEventName, specificHandler) {
        if (!socketEventName || !_.isString(socketEventName)) {
          throw new Error(
            'Invalid usage for `Cloud.off()`: Must pass in a first argument (a string; the name of the socket event to stop listening for -- i.e. the variety of incoming WebSocket messages to reject and ignore).'
          );
        }
        if (specificHandler !== undefined && !_.isFunction(specificHandler)) {
          throw new Error(
            'Invalid usage for `Cloud.off()`: If a second argument is provided, it should be a function  (the specific handler you want to stop running every time a matching WebSocket message is received).'
          );
        }

        if (!io || !io.socket) {
          throw new Error(
            'Could not stop listening to cloud events with `Cloud.off()`: WebSocket support is not currently available (`io.socket` is not available).  Make sure `sails.io.js` is being injected in a <script> tag!'
          );
        }
  ...
  function (global, factory) {
    ...
    if (global.location !== undefined) {
      if (
        ...
        (global.location.constructor.name === 'Location' ||
          global.location.constructor.toString() === '[object Location]' ||
          (_.isObject(global.location) && global.location.href))
      ) {
    ...
    if (global.FileList !== undefined) {
      if (
        global.FileList &&
        typeof global.FileList === 'function' &&
        global.FileList.name === 'FileList'
      ) {
    ...
    //‡AMD ˚¸
      ...
      // eslint-disable-line no-undef
      throw new Error(
        'Global `define()` function detected, but built-in AMD support in `cloud.js` is not currently recommended.  To resolve this, modify `cloud.js`.'
      );
    ...
    //‡NUDE ˚°·˛
      ...
      if (!global._) {
        throw new Error(
          "`_` global does not exist on the page yet. (If you're using Sails, please check dependency loading order in pipeline.js and make sure the Lodash library is getting brought in before `cloud`.)"
        );
      }
      _ = global._;
      // optional deps:
      if (global.io !== undefined) {
        if (typeof global.io !== 'function') {
          throw new Error(
            'Could not access `io.socket`: The `io` global is invalid at the moment:' +
              global.io +
      ...
      if (global.$ !== undefined) {
        if (typeof global.$ !== 'function') {
          throw new Error(
            'The `$` global is not valid at the moment:' +
              global.$ +
      ...
      if (global.SAILS_LOCALS !== undefined) {
        if (!_.isObject(global.SAILS_LOCALS)) {
          throw new Error(
            'The `SAILS_LOCALS` global is not valid at the moment:' +
              global.SAILS_LOCALS +
      ...
      if (global.Cloud) {
        throw new Error(
          'Cannot expose global variable: Conflicting global (`cloud`) already exists!'
        );
      }
